...
internal class VideoQuad
{
    ...
    public void PanTiltZoom(float pan, float tilt, float zoom, bool corrective_depth)
    {
        ...
        Vector3 offset = corrective_depth ? ((tilt_indent + pan_indent + _zoom) * _camera.transform.forward) : Vector3.zero;// - ((tilt_offset * _camera.transform.up) + (pan_offset * _camera.transform.right)) * 0.25f;
